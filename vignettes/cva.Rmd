...
### Estimate $t$ and $k$ with `cva_rank_trace()`
...
### Calculate residuals with `cva_residual()`
...
```{r}
cva_pairwise_plot(galaxy_x, galaxy_y, cv_pair = 1, k = 0.0001)
cva_pairwise_plot(galaxy_x, galaxy_y, cv_pair = 2, k = 0.0001)
```

```{r}
cva_pairwise_plot(galaxy_x, galaxy_y, cv_pair = 3, k = 0.0001)
cva_pairwise_plot(galaxy_x, galaxy_y, cv_pair = 6, k = 0.0001)
```
