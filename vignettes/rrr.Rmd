...
```{r}
rank_trace(tobacco_x, tobacco_y, ident, plot = FALSE)
```
...
```{r}
rank_trace(tobacco_x, tobacco_y, gamma2, plot = FALSE)
```
...
### Calculate Residuals with `rrr_residual()`

```{r}
args(rrr_residual_plot)
```

```{r}
rrr_residual(tobacco_x, tobacco_y, ident, rank = 1)
```

### Plot Residuals with `rrr_residual_plot()`

```{r}
args(rrr_residual)
```

We can visually check the model assumptions that 

```{r}
rrr_residual_plot(tobacco_x, tobacco_y, ident, rank = 1)
```